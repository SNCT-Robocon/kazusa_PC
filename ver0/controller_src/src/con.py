...
class UDPToCmdVel(Node):
    ...
    def __init__(self):
        ...
        # パブリッシャーとサブスクライバー
        self.pub = self.create_publisher(Controller, '/controller_val', qos_profile)
        self.sub = self.create_subscription(Pose2D, 'pose1', self.pose_callback, qos_profile)
        self.sub_str = self.create_subscription(String, '/log', self.str_callback, qos_profile)
        ...
        self.remote_addr = None  # 送信元のIPとポートを保持
        self.current_pose = {"x": 0.0, "y": 0.0, "theta": 0.0} # 最新のポーズ

        self.timer = self.create_timer(0.009, self.loop)
        self.get_logger().info("UDP node started - Waiting for packets...")

    def pose_callback(self, msg):
        """pose1トピックを受信したときに呼ばれる"""
        self.current_pose["x"] = msg.x
        self.current_pose["y"] = msg.y
        self.current_pose["theta"] = msg.theta
    def str_callback(self, msg):
        """/logを受信したら、その瞬間に1回だけUDP送信"""
    ...
    def loop(self):
        ...
        try:
            ...
            ctrl.baketu_turn_x= msg_dict.get("baketu_turnx", 0.0)
            ctrl.baketu_turn_y= msg_dict.get("baketu_turny", 0.0)
            ctrl.baketu_turn_theta= msg_dict.get("baketu_turntheta", 0.0)
            ctrl.hata_turn_x= msg_dict.get("hata_turnx", 0.0)
            ctrl.hata_turn_y= msg_dict.get("hata_turny", 0.0)
            ctrl.hata_turn_theta= msg_dict.get("hata_turntheta", 0.0)
            ctrl.hoju_turn_x= msg_dict.get("hoju_turnx", 0.0)
            ctrl.hoju_turn_y= msg_dict.get("hoju_turny", 0.0)
            ctrl.hoju_turn_theta= msg_dict.get("hoju_turntheta", 0.0)
            ctrl.hata_speed= msg_dict.get("hata_speed", 0.0)
            ctrl.baketu_speed= msg_dict.get("baketu_speed", 0.0)
            ctrl.hojuposition= msg_dict.get("HojuPosition", False)
            
            ctrl.fieldcolor= msg_dict.get("map_id", "none")
            
            self.pub.publish(ctrl)

            # ★ 最新の pose1 データを辞書形式にしてUDPで送り返す
            # 受信したコマンドに対するレスポンスとして送信
            response_json = json.dumps(self.current_pose)
